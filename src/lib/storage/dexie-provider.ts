...
export class DexieStorageProvider implements StorageProvider {
  ...
  setProgress(progress: MaterialProgress) {
    return this.db.progress.put(progress).then(() => undefined);
  }
  listProgress() {
  ...
  setNote(note: Note) {
    return this.db.notes.put(note).then(() => undefined);
  }
  ...
  addFeedback(feedback: Omit<Feedback, "id">) {
    return this.db.feedback.add(feedback as Feedback).then(() => undefined);
  }
  listFeedback() {
  ...
  setSimProgress(progress: SimProgress) {
    return this.db.simProgress
      .put({ ...progress, id: simProgressId(progress.scenarioId, progress.month) })
      .then(() => undefined);
  }
